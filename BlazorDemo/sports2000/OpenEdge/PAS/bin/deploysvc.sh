#!/bin/sh
#
# Shell wrapper to execute the PAS deploysvc utility
#
...
if [ "$DEPLOYSVC_JAR" = "" ] ; then
    # Fill in a default if the location was not already set
    # in the environment
    DEPLOYSVC_JAR="$CATALINA_HOME/bin/deploysvc*.jar"
fi

# Execute it now
# -Dorg.slf4j.simpleLogger.defaultLogLevel=trace
$JAVART/bin/java $JAVA_OPTS -Dcatalina.base=$CATALINA_BASE -jar $DEPLOYSVC_JAR ${1+"$@"}
