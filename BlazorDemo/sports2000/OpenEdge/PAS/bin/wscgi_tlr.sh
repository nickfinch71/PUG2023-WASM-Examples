...
if [ -z "$DLC" ]
then
    echo "***** DLC is not defined or points to an invalid location"
    exit 1
fi
...
if [ -z "$WRKDIR" ]
then
    echo "***** WRKDIR is not defined or points to an invalid location"
    exit 1
fi

# Test for the CGIIP executable 
if [ ! -f $DLC/bin/cgiip ]
then
    echo "***** WebSpeed/AppServer or WebSpeed Messenger is not installed"
    exit 1
fi
...
if [ -z "$CATALINA_HOME" ] 
then
    CATALINA_HOME="$DLC/servers/pasoe"
